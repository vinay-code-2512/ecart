...
def searchPage(Request):
    if Request.method=="POST":
        ...
        data = Product.objects.filter(Q(name__contains=search) | Q(color__contains=search) | Q(description__contains=search) | Q(size__contains=search) |  Q(stock__contains=search))
        maincategories = Maincategory.objects.all()
        subcategories = Subcategory.objects.all()
        brands = Brand.objects.all()
    ...
    if(p):
        ...
        if(cart):
            if(str(num) in cart):
                ...
                cart.setdefault(str(num),{'id':p.id,'name':p.name,'brand':p.brand.name,
                'color':p.color,'size':p.size,'price':p.finalprice,
                'qty':1,'total':p.finalprice,'pic':p.pic1.url})
        else:
           cart = {str(num):{'id':p.id,'name':p.name,'brand':p.brand.name,
           'color':p.color,'size':p.size,'price':p.finalprice,'qty':1,'total':p.finalprice,'pic':p.pic1.url}}
        subtotal = 0
        count = 0
    ...
    if(Request.method=="POST"):
        ...
        if(mode=="COD"):
            ...
            message = """
                    ...
                    Now you can track your order in your profile.
                    Team : E-Cart
                """
...
def paymentSuccessPage(Request,rppid,rpoid,rpsid,checkid):
    buyer = Buyer.objects.get(username=Request.user.username)
    check = Checkout.objects.get(id=checkid,buyer=buyer)      
    check.rppid=rppid
    ...
    if(Request.method=="POST"):
        if(Request.POST.get("password")!=Request.POST.get("cpassword")):
            ...
            try:
                ...
                message = """
                        Hello """+b.name+"""
                        Thanks to Create an Account on E-Cart.
                        Team : E-Cart
                        """
                email_from = settings.EMAIL_HOST_USER
    ...
    if(Request.method=="POST"):
        ...
        if(password==cpassword):
            try:
                ...
                messages.error(Request,"Unathorized")    
        else:
